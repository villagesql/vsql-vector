...
struct ColumnStore {
  ...
  // it is ignored (and defaults to empty) when m_rowid_max == 0. It is the
  // trailing, defaulted argument so the HNSW graph level stores (m_rowid_max ==
  // 0) keep calling insert() with their original argument list unchanged.
  bool insert(MtrCtx::Ref mctx, Segment::TrxRef trx_ref, Column::Data col_data,
              ...
              Column::Data rowid_prefix = Column::Data{});
